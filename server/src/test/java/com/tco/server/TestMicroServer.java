...
public class TestMicroServer {
    ...
    @Test
    @DisplayName("tmiao: Valid distance request succeeds with 200 status")
    public void testValidDistancesRequest() throws IOException {
        String validDistancesRequestJSON = 
            "{\"requestType\":\"distances\", \"earthRadius\":1.0, \"places\": [] }";
        HttpResponse response = postRequest("/api/distances", validDistancesRequestJSON);
        assertEquals(200, response.getStatusLine().getStatusCode());
    }
    @Test
    @DisplayName("joseph32: Valid find request succeeds with 200 status")
    public void testValidFindRequest() throws IOException {
        String validFindRequestJSON = 
            "{\"requestType\":\"find\", \"match\":\"\", \"limit\": 1 }";
        HttpResponse response = postRequest("/api/find", validFindRequestJSON);
        assertEquals(200, response.getStatusLine().getStatusCode());
    }

    @Test
    @DisplayName("lwyip: Valid tour request succeeds with 200 status")
    public void testValidTourRequest() throws IOException {
        String validTourRequestJSON = 
            "{\"requestType\":\"tour\",\"earthRadius\":1.0,\"response\":1, \"places\":[] }";
        HttpResponse response = postRequest("/api/tour", validTourRequestJSON);
        assertEquals(200, response.getStatusLine().getStatusCode());
    }
